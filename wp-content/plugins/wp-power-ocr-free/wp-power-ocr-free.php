			...
			foreach($_POST['images'] as $key => $file_url)
			{
				if($file_url != '')
				{
					...
					if(strstr($file_url, '.jpg') || strstr($file_url, '.png') || strstr($file_url, '.gif') || strstr($file_url, '.bmp'))
						echo '<img src="'.$file_url.'" /><br />';
					else
						echo '<a href="'.$file_url.'" target="_blank">'.$file_url.'</a><br />';
					...
					{
						echo '<blockquote>'.sanitize_text_field($data['text']).'</blockquote>';
					}
					else
						echo '<p>Error : '.$data['message'].'</p>';//.print_r($data, true);
